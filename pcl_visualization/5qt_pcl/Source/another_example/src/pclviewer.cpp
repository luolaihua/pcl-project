...
PCLViewer::PCLViewer(QWidget* parent) :
    QMainWindow(parent), ui(new Ui::PCLViewer_UI), filtering_axis_(1),  // = y
...
void PCLViewer::loadFileButtonPressed() {
    ...
    for (PointCloudT::iterator cloud_it = cloud_->begin(); cloud_it != cloud_->end(); ++cloud_it) {
        //        qDebug() << cloud_it->_PointXYZRGBA::r << " " << cloud_it->_PointXYZRGBA::g << " " <<
...
void PCLViewer::colorCloudDistances() {
    ...
    for (PointCloudT::iterator cloud_it = cloud_->begin(); cloud_it != cloud_->end(); ++cloud_it) {
        ...
        switch (filtering_axis_) {
            case 0:  // x
                value =
                    boost::math::iround((cloud_it->x - min) * lut_scale);  // Round the number to the closest integer
                break;
            case 1:  // y
        ...
        switch (color_mode_) {
            ...
            default:
                ...
                cloud_it->g =
                    value < 128 ? 2 * value : 255 - ((value - 128) * 2);  // g[0] = 0, g[128] = 255, g[255] = 0
